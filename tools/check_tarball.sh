...
#
# Two real incidents in sibling packages:
...
#
# It matters a little more here than in a text-only package: stt.api ships
# binary audio in inst/audio, so tarball weight is no longer something a
# glance at the diff would reveal.
#
...
# Exactly what stt.api ships today. Deliberately tight rather than the
